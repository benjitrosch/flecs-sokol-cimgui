...
/*
    ...
    SOKOL_IMGUI_NO_SOKOL_APP    - don't depend on sokol_app.h (see below for
   details)
   ...
   saving UI state while using your own filesystem wrapper functions (in this
            ...
            bool disable_paste_override
                ...
                copy/paste support isn't properly fleshed out in sokol_imgui.h
   yet.
            ...
            bool disable_windows_resize_from_edges
                If true, windows can only be resized from the bottom right
   corner. The default is false, meaning windows can be resized from edges.
    ...
    --- At the start of a frame, call:
        ...
        simgui_new_frame(&(simgui_frame_desc_t){.width = ..., .height = ...,
   .delta_time = ..., .dpi_scale = ...});
        ...
        'dpi_scale' is the current DPI scale factor, if this is left
   zero-initialized, 1.0f will be used instead. Typical values for dpi_scale are
   >= 1.0f.
   ...
   framebuffer:
    ...
    --- if you're using sokol_app.h, from inside the sokol_app.h event callback,
        ...
        Note that simgui_map_keycode() can be called outside
   simgui_setup()/simgui_shutdown().
...
extern "C"
{
    ...
    */
    ...
    typedef struct simgui_desc_t
    {
        ...
        const char*     ini_filename;
        bool            no_default_font;
        bool disable_paste_override;    // if true, don't send Ctrl-V on
                                        // EVENTTYPE_CLIPBOARD_PASTED
        bool disable_set_mouse_cursor;  // if true, don't control the mouse
                                        // cursor type via
                                        // sapp_set_mouse_cursor()
        bool disable_windows_resize_from_edges;  // if true, only resize edges
                                                 // from the bottom right corner
        bool write_alpha_channel;  // if true, alpha values get written into the
                                   // framebuffer
        simgui_allocator_t allocator;  // optional memory allocation overrides
                                       // (default: malloc/free)
    } simgui_desc_t;
    ...
    SOKOL_IMGUI_API_DECL int  simgui_map_keycode(sapp_keycode keycode
     );  // returns ImGuiKey_*
#endif
...
inline void simgui_setup(const simgui_desc_t& desc)
{
    return simgui_setup(&desc);
}
inline void simgui_new_frame(const simgui_frame_desc_t& desc)
{
    return simgui_new_frame(&desc);
}
...
#error \
    "SOKOL_MALLOC/CALLOC/FREE macros are no longer supported, please use simgui_desc_t.allocator to override memory allocation functions"
#endif
...
static void* _simgui_malloc(size_t size)
{
    ...
    {
        ptr = _simgui.desc.allocator.alloc(
            size, _simgui.desc.allocator.user_data
        );
    }
    else
    {
...
{
    SOKOL_ASSERT(
        (desc->allocator.alloc && desc->allocator.free) ||
        (!desc->allocator.alloc && !desc->allocator.free)
    );
...
SOKOL_API_IMPL void simgui_setup(const simgui_desc_t* desc)
{
    ...
    */
    ...
    _simgui.vertices.size =
        (size_t)_simgui.desc.max_vertices * sizeof(ImDrawVert);
    _simgui.vertices.ptr = _simgui_malloc(_simgui.vertices.size);
    _simgui.indices.size =
        (size_t)_simgui.desc.max_vertices * 3 * sizeof(ImDrawIdx);
...
/* initialize Dear ImGui */
    ...
    io->ConfigWindowsResizeFromEdges =
        !_simgui.desc.disable_windows_resize_from_edges;
    ...
    if (!_simgui.desc.no_default_font)
    {
        ...
        ImFontAtlas_GetTexDataAsRGBA32(
            io->Fonts, &font_pixels, &font_width, &font_height, &bytes_per_pixel
        );
#endif
    ...
    pip_desc.colors[0].write_mask =
        _simgui.desc.write_alpha_channel ? SG_COLORMASK_RGBA : SG_COLORMASK_RGB;
    ...
    */
         ...
         cl_index++, cmd_list_count++)
    {
        ...
        if (vtx_size > 0)
        {
            ...
            void*             dst_vtx_ptr =
                (void*)(((uint8_t*)_simgui.vertices.ptr) + all_vtx_size);
        ...
        if (idx_size > 0)
        {
            ...
            void*            dst_idx_ptr =
                (void*)(((uint8_t*)_simgui.indices.ptr) + all_idx_size);
    ...
    for (int cl_index = 0; cl_index < cmd_list_count; cl_index++)
    {
        ...
        for (int cmd_index = 0; cmd_index < num_cmds; cmd_index++)
        {
            ...
            if (pcmd->UserCallback)
            {
                ...
                sg_apply_pipeline(_simgui.pip);
                sg_apply_uniforms(
                    SG_SHADERSTAGE_VS, 0, SG_RANGE_REF(vs_params)
                );
                sg_apply_bindings(&bind);
            }
            else
...
_SOKOL_PRIVATE void _simgui_add_mouse_button_event(
    ImGuiIO* io, int mouse_button, bool down
)
{
#if defined(__cplusplus)
...
_SOKOL_PRIVATE void _simgui_add_mouse_wheel_event(
    ImGuiIO* io, float wheel_x, float wheel_y
)
{
#if defined(__cplusplus)
...
_SOKOL_PRIVATE void _simgui_add_sapp_key_event(
    ImGuiIO* io, sapp_keycode sapp_key, bool down
)
{
...
_SOKOL_PRIVATE void _simgui_add_imgui_key_event(
    ImGuiIO* io, ImGuiKey imgui_key, bool down
)
{
#if defined(__cplusplus)
    io->AddKeyEvent(imgui_key, down);
...
{
    _simgui_add_imgui_key_event(
        io, ImGuiKey_ModCtrl, (mods & SAPP_MODIFIER_CTRL) != 0
    );
    _simgui_add_imgui_key_event(
        io, ImGuiKey_ModShift, (mods & SAPP_MODIFIER_SHIFT) != 0
    );
    _simgui_add_imgui_key_event(
        io, ImGuiKey_ModAlt, (mods & SAPP_MODIFIER_ALT) != 0
    );
    _simgui_add_imgui_key_event(
        io, ImGuiKey_ModSuper, (mods & SAPP_MODIFIER_SUPER) != 0
    );
}
    ...
    switch (ev->type)
    {
        ...
        case SAPP_EVENTTYPE_MOUSE_DOWN:
            ...
            _simgui_add_mouse_pos_event(
                io, ev->mouse_x / dpi_scale, ev->mouse_y / dpi_scale
            );
...
// "platform behaviour flags".
        ...
        case SAPP_EVENTTYPE_TOUCHES_BEGAN:
            ...
            _simgui_add_mouse_pos_event(
                io, ev->touches[0].pos_x / dpi_scale,
